...
- [ ] **#219 — Verify `meriyah` resolves to 7.x in the deployed tree; widen the parser probe** _(opened 2026-07-30 from the #218 review; deps: none, but it is a release gate for #200. The runtime lib pins `^7.2.0` while a transitive `meriyah@1.9.15` sits at the root, so npm nested the 7.x copy — the pruned production resolution is unverified. Since D-AM made analysis fail closed, a wrong-major resolution now refuses plugin installs outright. The probe as landed passes on 1.x and only fails on modern syntax, so it does not detect the case it was written for.)_
...
#193 ─┬─▶ #59 ◀── #60          #198 ─▶ #199 ◀─ (#59, #84)
      ...
      │    └─▶ #206 ◀─ #84
      └─▶ #200 ◀── #219 (meriyah resolution: release gate)
...
- 2026-07 amendments to #59/#84/#211/#216 live as dated issue comments (API body edits mangle code fences); latest: 2026-07-30 C2 as-merged decisions (D-AL–D-AM) on #59. Fold into the bodies on next manual edit. #60's body IS current (rewritten 2026-07-22; later status flags are comments).
